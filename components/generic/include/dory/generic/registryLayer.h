...
namespace dory::generic::registry
{
    ...
    {
        static constexpr std::size_t value = ValueListLength<typename TService::IdentifierListType>::value + ServiceCountTraverse<TServices...>::value;
    };
    ...
    template<typename... TServices>
    struct ServiceCount<TypeList<TServices...>>
    {
    ...
    {
        using NextServiceTraverseType = ServiceIndexTraverse<Index + ValueListLength<typename TService::IdentifierListType>::value, TServiceInterface, TServices...>;
    ...
    struct ServiceIndex<TServiceInterface, Identifier, TypeList<TServices...>>
    {
        using IndexTraverseType = ServiceIndexTraverse<0, TServiceInterface, TServices...>;
        static constexpr int offset = ValueIndex<Identifier, typename IndexTraverseType::ServiceEntryType::IdentifierListType>::value;
    ...
    struct ServiceIndexRuntime<TServiceInterface, TypeList<TServices...>>
    {
        using IndexTraverseType = ServiceIndexTraverse<0, TServiceInterface, TServices...>;
        ...
        {
            return value + ValueIndexRuntime<typename IndexTraverseType::ServiceEntryType::IdentifierListType>::get(searchValue);
        }
    };
    ...
    template<typename TServiceInterface, typename... TServices>
    struct ServiceIndexAnchor<TServiceInterface, TypeList<TServices...>>
    {
        using IndexTraverseType = ServiceIndexTraverse<0, TServiceInterface, TServices...>;
    ...
    private:
        ...
        using ServiceHandleType = extension::ResourceHandle<ServicePointerType<BaseInterfaceType>>;
    ...
    private:
        ...
        constexpr int getServiceEntryIndex()
        {
            ...
            static_assert(index >= 0 && index < ServiceCount<TServiceList>::value); //Index is out of range
        ...
        constexpr int getServiceEntryIndex(TIdentifier identifier)
        {
            ...
            assert::debug(index >= 0 && index < Length<TServiceList>::value, "Index is out of range");

            return index;
        }

        template<typename TServiceInterface>
        void set_internal(std::size_t index, const extension::LibraryHandle& libraryHandle, const ServicePointerType<TServiceInterface>& service)
        {
            std::shared_ptr<BaseInterfaceType> ptr = std::static_pointer_cast<TServiceInterface>(service);
            _services[index] = extension::ResourceHandle{ libraryHandle, ptr };
        }
        ...
        void invoke(TAction&& action, extension::ResourceRef<std::shared_ptr<TServiceInterface>>... serviceRefs)
        {
            if((serviceRefs && ...))
            {
    ...
    public:
        ...
        template<typename TServiceInterface>
        void set(const extension::LibraryHandle& libraryHandle, const ServicePointerType<TServiceInterface>& service)
        {
        ...
        template<typename TServiceInterface, typename TIdentifier>
        void set(const extension::LibraryHandle& libraryHandle, const ServicePointerType<TServiceInterface>& service, TIdentifier&& identifier)
        {
        ...
        void getAll(A&& action)
        {
            ...
            constexpr auto identifiers = ValueArray<typename ServiceAnchorType::ServiceEntryType::IdentifierListType>::get();
