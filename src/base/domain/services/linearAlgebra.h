...
namespace dory::domain::services
{
    ...
    template<typename T>
    class MatrixCalculator<geometry::Matrix<T, geometry::Matrix4x4Size>>
    {
    public:
        using MatrixType = geometry::Matrix<T, geometry::Matrix4x4Size>;
        using Point3Type = geometry::Point<T, std::integral_constant<std::size_t, 3>>;

        void setToIdentity(MatrixType& matrix)
        {
        ...
        void copyEntriesTo(const MatrixType& sourceMatrix, MatrixType& destinationMatrix)
        {
            for(int i = 0; i < geometry::Matrix4x4Size::value; ++i)
            {
                destinationMatrix.entries[i] = sourceMatrix.entries[i];
            }
        }

        void multiply(const MatrixType& matrixLeftSide, const MatrixType& matrixRightSide, MatrixType& resultMatrix)
        {
            const auto& lhs = matrixLeftSide.entries;
            const auto& rhs = matrixRightSide.entries;
        ...
        void multiply(const MatrixType& matrix, const Point3Type& vector, Point3Type& resultVector)
        {
        ...
        void multiply(const MatrixType& matrix, const Point3Type& vector, Point3Type& resultVector, T& wResult)
        {
        ...
        void invert(const MatrixType& matrix, MatrixType& resultMatrix)
        {
            const auto& m = matrix.entries;
            ...
            if (det) {
                auto& result = resultMatrix.entries;
        ...
        void toIdentity(MatrixType& matrix)
        {
            MatrixType::setEntries(matrix, MatrixType::EntriesArray({
                    1.f, 0.f, 0.f, 0.f,
                    0.f, 1.f, 0.f, 0.f,
                    0.f, 0.f, 1.f, 0.f,
                    0.f, 0.f, 0.f, 1.f
        ...
        void translate(MatrixType& matrix, T dx, T dy, T dz)
        {
            matrix.entries[3] = dx;
            matrix.entries[7] = dy;
        ...
        void scale(MatrixType &matrix, T x, T y, T z)
        {
            matrix.entries[0] = x;
            matrix.entries[5] = y;
            matrix.entries[10] = z;
        ...
        void rotateX(MatrixType& matrix, float radians)
        {
        ...
        void rotateY(MatrixType& matrix, float radians)
        {
        ...
        void rotateZ(MatrixType& matrix, float radians)
        {
            matrix.entries[0] = std::cos(radians);
        ...
        bool isEqual(const MatrixType& matrixA, const MatrixType& matrixB)
        {
            for(int i = 0; i < geometry::Matrix4x4Size::value; ++i)
            {
    ...
    using MatrixCalculator4f = MatrixCalculator<geometry::Matrix4x4f>;
